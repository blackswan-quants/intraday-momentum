...
class AccurateIntradayMomentum(QCAlgorithm):
    ...
    def OnData(self, data: Slice):
        ...
        if sigma == 0 and not self.IsWarmingUp:
            # Update stats for future days even if we don't trade today
...
# 6. Trade Execution Logic (Every 30 Minutes)
        if current_time.minute % 30 == 0:
            ...
            if not self.Portfolio.Invested:
                # Calculate target leverage (e.g., 2.5 for 250% leverage)
                target_leverage = self.CalculateDynamicSize()
                if target_leverage == 0: return

                if current_price > upper_bound:
                    # SetHoldings automatically handles the quantity math
