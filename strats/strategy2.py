...
class IntradayMomentum_2(QCAlgorithm):
    def Initialize(self):
        # 1. Setup Basics
        ...
        self.ema_period = 100
        self.ema = self.EMA(self.spy_symbol, self.ema_period, Resolution.Minute)
    ...
    def OnData(self, data: Slice):
        ...
        # 2. Safety Check: Need Open, Prev Close, Volatility Data, and EMA ready
        if (
        ...
        # sigma = average of the same minute's absolute move over last 14 days
        ...
        # 6. Trade Execution Logic (Every 30 Minutes)
        if current_time.minute % 30 == 0:
            ...
            if not self.Portfolio.Invested:
                ...
                # Entry only if breakout is also confirmed by VWAP and EMA
                if current_price > upper_bound and current_price > vwap_val and current_price > ema_val:
                    self.SetHoldings(self.spy_symbol, target_leverage)

                elif current_price < lower_bound and current_price < vwap_val and current_price < ema_val:
                    self.SetHoldings(self.spy_symbol, -target_leverage)

            else:
                # Dynamic Trailing Stop / Exit Logic
    ...
    def CalculateDynamicSize(self):
        ...
        # Target Vol / Current Vol = Leverage (capped at 2)
