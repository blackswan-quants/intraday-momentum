...
class Analyzer:
    ...
    def __init__(
        ...
        self.max_workers = max(1, min(cpu_count, 8)) if max_workers is None else max(1, max_workers)
    ...
    def compute_perf_stats(self, daily_pnl_df: pd.DataFrame) -> dict:
        ...
        annualized_return = (np.prod(1 + returns) ** (self.trading_days / len(returns)) - 1) * 100
        vol = returns.std()
        annualized_vol = vol * np.sqrt(self.trading_days) * 100
        hit_ratio = (returns > 0).sum() / len(returns) * 100
        # Compute Sharpe ratio with guard against zero volatility to avoid division error
        if vol < 1e-9:
            sharpe_ratio = np.nan
        else:
            sharpe_ratio = (returns.mean() / vol) * np.sqrt(self.trading_days)
        ...
        if spy_returns is not None:
            ...
            if spy_returns.std() < 1e-9:
                ...
                if len(aligned_df) > 0:
                    ...
                    try:
                        alpha = ( (1.0 + alpha_daily) ** self.trading_days - 1.0 ) * 100.0
                    except Exception:
                        ...
                        alpha = alpha_daily * self.trading_days * 100.0
        ...
        if 'AUM' in daily_pnl_df.columns:
            ...
            if np.isfinite(first_aum) and first_aum > 0.0:
                cumulative_return = daily_pnl_df['AUM'] / first_aum
            else:
                logging.warning(
                    "AUM is present but the first value is invalid (<=0 or non-finite). "
        ...
        # Helper function to annotate certain metrics for readability in logs
    ...
    def sensitivity_sweep(self, param_grid: dict, backtest_fn=None) -> pd.DataFrame:
        ...
        logging.info(f"max_workers={self.max_workers} (cpu_count={os.cpu_count() or 1}, cap=8)")
...
def compute_perf_stats(daily_pnl_df: pd.DataFrame) -> dict:
    ...
    return Analyzer().compute_perf_stats(daily_pnl_df)
